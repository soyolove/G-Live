...
interface TTSRequest {
  ...
  app: {
    appid: string;      // 应用 ID
    token: string;      // 访问令牌（其实是假的，随便填）
    cluster: string;    // 集群名称（固定为 'volcano_tts'）
  };
  
  // 用户信息
  user: {
    uid: string;        // 用户标识（随便填，用于日志追踪）
  };
  
  // 音频参数
  audio: {
    voice_type: string;                              // 音色类型
    encoding: 'mp3' | 'wav' | 'pcm' | 'ogg_opus';   // 音频格式
    speed_ratio?: number;                            // 语速（0.8-2.0）
    rate?: number;                                   // 采样率
    loudness_ratio?: number;                         // 音量（0.5-2.0）
  };
  
  // 请求参数
  request: {
    reqid: string;              // 请求 ID（必须唯一，用 UUID）
    text: string;               // 要转语音的文本
    operation: 'query';         // 操作类型（HTTP 只能用 'query'）
    silence_duration?: number;  // 句尾静音时长（毫秒）
    with_timestamp?: number;    // 是否返回时间戳
  };
}
...
interface TTSResponse {
  reqid: string;       // 请求 ID（和请求中的一样）
  code: number;        // 状态码（3000 表示成功）
  operation: string;   // 操作类型
  message: string;     // 状态信息
  sequence: number;    // 序列号（-1 表示完成）
  data: string;        // 音频数据（base64 编码的二进制数据）
  addition?: {         // 额外信息
...
async function generateSpeech(text: string, outputPath: string): Promise<void> {
  try {
    ...
    const request: TTSRequest = {
      ...
      // 请求内容
      request: {
        reqid: uuidv4(),          // 唯一请求 ID（用 UUID 生成）
        text: text,               // 要转语音的文本
...
// 导出函数供其他模块使用
// 这样其他文件可以通过 import { generateSpeech } from './tts-test' 来使用
export { generateSpeech, TTS_CONFIG };
